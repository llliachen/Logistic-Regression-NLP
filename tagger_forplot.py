...
def print_llh_train(avr_llg):
    f = open('model2_train.txt', 'a')
    f.write('%.6f, ' % (avr_llg));
    f.close()

def print_llh_validation(avr_llg):
    f = open('model2_validation.txt', 'a')
    f.write('%.6f, ' % (avr_llg));
    f.close()
...
for epc in range(num_epoch):
    ...
    print_llh_train(avr_llh_train)
    print_llh_validation(avr_llh_vldat)
