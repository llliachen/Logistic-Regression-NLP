...
def sgd_mode2(is_x, y_i):
    ...
    for k in range(K):
        ...
        factor = -((1 if k == y_i else 0) - exp_theta_k_x_i / sum_exp)
        deriv_theta_k = [0 for col in range(3*M)]
        deriv_theta_k[0] = factor
        for i in range(len(is_x)):
            deriv_theta_k[is_x[i]] = factor
        deriv_theta.append(deriv_theta_k)

    eta_v = [eta for col in range(3*M)]
    for k in range(K):
        theta[k] = theta[k] - np.multiply(eta_v, deriv_theta[k])
...
if featr_flag == 1:
    ...
    data_train, train_seq_idx = read_feature(train_file)
    data_train_mode_2 = parse_feature(data_train, train_seq_idx)

# initialize parameters
data_vldat, vldat_seq_idx = get_data(vldat_file)
data_test, test_seq_idx = get_data(test_file)
if featr_flag == 2:
    data_test_mode_2 = parse_feature(data_test, test_seq_idx)
    data_vldat_mode_2 = parse_feature(data_vldat, vldat_seq_idx)
